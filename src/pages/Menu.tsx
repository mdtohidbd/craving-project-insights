          ...
          >
            {isLoading && categories.length === 0 ? (
              Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="flex-shrink-0 px-5 py-2 rounded-full bg-zinc-200/50 animate-pulse w-24 h-8"></div>
              ))
            ) : (
      ...
      <div className="container mx-auto px-6 md:px-12 pb-20">
        {isLoading && categories.length === 0 ? (
          <div className="pt-10 pb-6">
            <div className="w-48 h-10 bg-zinc-200/50 rounded mx-auto mb-10 animate-pulse"></div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
              {Array.from({ length: 6 }).map((_, index) => {
                const shapeClass = getShapeClass(index);
                return (
                  <div key={`skeleton-initial-${index}`} className="flex flex-col items-center bg-transparent mb-6 h-full animate-pulse p-4">
